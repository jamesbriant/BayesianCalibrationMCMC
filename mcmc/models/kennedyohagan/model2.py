...
class Model(KennedyOHagan):
    ...
    def calc_prior(self, param: Parameter) -> None:
        ...
        if param.name == "theta":
            ...
            omega_eta = -4*np.log(param.values.copy())      #copy() prevents overwriting the parameter values!
            omega_eta[omega_eta > 0.999] = 0.999
            for index, value in enumerate(omega_eta):
                param.prior_densities[index] = beta.logpdf(value, 1, 0.5, loc=0, scale=1)
        elif param.name == "omega_delta":
            omega_delta = -4*np.log(param.values.copy())    #copy() prevents overwriting the parameter values!
    ...
    def calc_sigma_eta(self, data: Data) -> None:
        ...
        self._sigma_eta[self.I_eta] = np.exp(-self.D_eta @ self.params['omega_eta'].values)/self.params['lambda_eta'].values
